...
func (p *Requset) FastRead(buf []byte) (int, error) {
	var err error
	var offset int
	var l int
...
ReadFieldError:
	return offset, thrift.PrependError(fmt.Sprintf("%T read field %d '%s' error: ", p, fieldId, fieldIDToName_Requset[fieldId]), err)
SkipFieldError:
...
func (p *Requset) FastReadField1(buf []byte) (int, error) {
	offset := 0
...
// for compatibility
func (p *Requset) FastWrite(buf []byte) int {
	return 0
}

func (p *Requset) FastWriteNocopy(buf []byte, binaryWriter bthrift.BinaryWriter) int {
	offset := 0
	offset += bthrift.Binary.WriteStructBegin(buf[offset:], "Requset")
	if p != nil {
...
func (p *Requset) BLength() int {
	l := 0
	l += bthrift.Binary.StructBeginLength("Requset")
	if p != nil {
		l += p.field1Length()
	}
...
func (p *Requset) fastWriteField1(buf []byte, binaryWriter bthrift.BinaryWriter) int {
	offset := 0
...
func (p *Requset) field1Length() int {
	l := 0
